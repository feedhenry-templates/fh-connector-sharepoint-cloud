...
function getParamsFromEnvVariables(){
  return {
    ...
    type : process.env.SP_AUTH_TYPE || 'basic',
    url : process.env.SP_HOSTNAME,
    strictSSL : (process.env.SP_STRICT_SSL === 'true')
  };
}
  ...
  if (sessionToken){
    return fh.session.get(sessionToken, function(err, data){
      ...
      try{
        ...
        console.log("Error parsing data from session store");
        return cb(err);
      }
      data.type = data.type || process.env.SP_AUTH_TYPE || 'basic';
      data.type = data.type || process.env.SP_AUTH_TYPE || 'basic';
      data.url = data.url || process.env.SP_HOSTNAME;
      data.strictSSL = data.strictSSL || process.env.SP_STRICT_SSL;
      return cb(null, data);
    });
  }
  // No session set - revert to env variables
  var params = getParamsFromEnvVariables();
  params.username = process.env.SP_USERNAME;
  params.password = process.env.SP_PASSWORD;
...
exports.set = function(req, cb){
  var params = req.body,
  sessionId = uuid.v1(),
  sessionExpire = process.env.SP_SESSION_TIMEOUT || 60 * 60 // 60 minutes,
  sharepointer;
  ...
  try{
    sharepointer = require('sharepointer')(params);
  }catch(err){
    return cb(err);
  }
  console.log('initted sp with');
  console.log(params);
